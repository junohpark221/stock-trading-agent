...
- [매매 로직 레퍼런스](./TRADING_LOGIC.md) — 매매 전략, 분석 차트 생성 로직 통합 문서 (Phase 2 기술/패턴/펀더멘털 작성 완료)
...
- [x] `src/data/providers/ecos_provider.py` ECOS 매크로 경제 지표 제공자
- [x] `src/data/providers/fred_provider.py` FRED 미국 경제 지표 제공자
- [x] `src/data/providers/naver_provider.py` 네이버 뉴스 데이터 제공자 (수집만, 감성분석은 Phase 3)
- [x] `src/db/models/analysis.py` Phase 2 DB 모델 4개 + 마이그레이션
- [x] `src/analysis/technical/indicators.py` 기술 지표 (pandas-ta, 상세 주석 포함)
- [x] `src/analysis/technical/patterns.py` 차트 패턴 감지 (상세 주석 포함)
- [x] `src/analysis/fundamental/analyzer.py` 펀더멘털 분석 (DART 연동)
- [x] `src/data/utils/market_helpers.py` pykrx 유틸리티 함수
- [x] `src/api/routes/analysis.py` 분석 결과 API 엔드포인트
- [x] `docs/TRADING_LOGIC.md` 매매 로직 통합 레퍼런스 작성
- [x] DART 공시/재무제표 수집 테스트
- [x] 기술 지표 단위 테스트 + 차트 패턴 테스트
- [x] 펀더멘털 점수 산출 테스트 (DART 데이터 포함)
- [x] 뉴스 수집 테스트 (네이버 API)
- [x] ECOS/FRED 매크로 지표 수집 테스트
